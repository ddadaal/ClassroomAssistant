...
- [3. P2P架构设计](#3-p2p%E6%9E%B6%E6%9E%84%E8%AE%BE%E8%AE%A1)
  ...
  - [3.6 第六次迭代（签到模块）](#36-%E7%AC%AC%E5%85%AD%E6%AC%A1%E8%BF%AD%E4%BB%A3%E7%AD%BE%E5%88%B0%E6%A8%A1%E5%9D%97)
...
- [7. 接口和实现映射关系](#7-%E6%8E%A5%E5%8F%A3%E5%92%8C%E5%AE%9E%E7%8E%B0%E6%98%A0%E5%B0%84%E5%85%B3%E7%B3%BB)
  - [7.1 服务器端](#71-%E6%9C%8D%E5%8A%A1%E5%99%A8%E7%AB%AF)
    ...
    - [7.1.7 文件模块](#717-%E6%96%87%E4%BB%B6%E6%A8%A1%E5%9D%97)
...
将**服务器**分解为**考试模块、讨论模块、举手模块、签到模块、文件模块、消息推送模块、网络模块、权限管理模块、数据库模块、登录模块** 
...
| ---- | ----------- | ------ | ------ |
|1     |场景6：断线重连|高|高|
|2|场景1：大量用户短时间内进行数据量交换较小的操作|中|中|
...
| 2    | 防止数据非法访问  | 加密文件                          | 设计约束1 |
| 3    | 资源数据传输协议 | TCP协议                          | 文件上传和下载、用户请求服务           |
| 4    | 资源传输效率     | 镜像服务器加速和离线下载         | 文件上传和下载、用户请求服务           |
| 5    | 中断处理和恢复   | 保存本地备份，重新下载时导入恢复 | 文件上传和下载、断线重连、用户请求服务 |
| 6    | 上传数据处理方式 | 只解压一小部分文件，检查是否有病毒后存储     | 文件上传和下载、用户请求服务                                 |
| 7    | 上传数据处理效率 | 使用RAD5存储上传的数据、边解压边分析数据     | 文件上传和下载                                               |
| 8    | 上传断点记录     | 自定义断点交互协议                           | 文件上传和下载、断线重连                                     |
| 9  | 上传数据拆分方式 | 定义拆分单元为1M                             | 文件上传和下载 |
...
| 2    | 提高权限检查速度 | 请求队列、多线程并行       | 系统快速响应用户操作、用户请求服务                                   |
...
| 2| 短时间内支持多个用户 | 提升服务器性能 | 场景1，场景2，设计约束2 |
...
选择模式：**提升服务器性能**
选择理由：提升服务器性能和DNS负载均衡都可以显著提升系统处理高并发的能力，但DNS负载均衡开发和成本难度较大，适用于大型互联网产品，在本教学支持系统中不实用，故不选取。
...
| 2    | 支持高并发   | 提升服务器性能    | 场景1，场景2，场景10，设计约束2 |
...
| 3    | 场景3：由于各种原因导致的系统出错                            | 高     | 高     |
| 4    | 场景6：断线重连                                              | 高     | 高     |
| 5    | 场景7：学生数据防止被攻击和篡改                              | 高     | 高     |
| 6    | 场景10：用户请求服务                                         | 中     | 中·    |
| 7    | 场景2：系统快速响应用户操作                                          | 高     | 高     |
...
|优缺点 |P2P|C/S|
|---|---|---|
|优点|P2P网络规模越大，网络性能越高。|目前硬件和软件技术的发展已经能够简单满足数百人同时在线的需求。|
|缺点|节点的处理能力参差不齐（比如手机），当用户节点性能较低时，大量的计算会影响用户的使用体验|服务器是系统性能瓶颈所在，当达到服务器性能上限时，所有用户的使用体验都会受到影响|
...
|缺点|架构复杂，开发和维护成本较高。<br/>系统冷启动极为麻烦。<br/>一些功能（例如随时可用地共享文件功能）实现复杂度极高|横向扩展难度较高，且会增加系统的复杂度，增加维护的复杂度。|
...
|---|---|---|
| 优点 |  | 符合用户使用应用的直觉 |
| 缺点 | 需要指导用户使用他们不熟悉的P2P连接技术，成本高 | 可能会消耗用户的网络流量（部署于互联网） |

## 5.5 成本对比 

| 优缺点 | P2P | C/S |
| -- | -- | -- |
| 优点 | 不需要维护中心服务器，或者中心服务器功能简单，成本极低 | 由于架构简单成熟，开发维护人力成本较低 |
| 缺点 | P2P架构对用户设备的性能和硬件配置有一定需求<br/> | 需要维护中心服务器 |

## 5.6 最终选择

综合考虑性能（服务器端能够简单地满足数百人在线地需求）、安全性（服务器端安全策略已经成熟）、复杂度（CS架构更加简单）、易用性（符合用户直觉）和成本（硬件成本正逐年降低，且熟悉CS架构的人力成本更加便宜）因素，最终我们选择使用**CS架构**。

# 6. 类图

由于Demo实现是采用RMI技术实现的，客户端和服务器端通过共用Remote接口进行通信，所以项目分成三个包（server、client和shared），其中shared包包含共用接口和数据结构。所以在目前的Demo中，即使已经让客户端和服务器端的通信经过网络模块，**客户端的对应模块将会直接依赖服务器端的远程接口**，而不是像CC图中一样两端都通过网络模块转发。在真实的CS项目中，两端可采用RESTful或者GraphQL等更加语言无关的通信协议，这样服务器端即可以做到让网络模块转发服务，这样就完全符合之前的C&C图了。
